...
MAN_String* SplitWords(MAN_String* string_vector, char* string, int* len_MAN_String)
{
    ...
    for(int index = 0; index < strlen(string); index++)
    {
        ...
        if(EndOfWord(string[index])){
            ...
            memcpy(string_vector[*len_MAN_String].word, ptr_start, index - pos_ptr_start); // copia exatamente o numero de bytes dados pelo index ,da palavra que start aponta para dentro da posição atual de inserção do vetor strings
            if(index + 1 < strlen(string)){ // caso exista posições maiores que index + 1, existem outras palavras
                while(EndOfWord(ptr_start[index - pos_ptr_start +1]))
                    index++;
                ...
                string_vector = realloc(string_vector,(*len_MAN_String + 1) * sizeof(MAN_String)); // raloca espaço para mais uma posição 
